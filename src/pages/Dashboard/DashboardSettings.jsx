...
function DashboardSettings() {
    ...
    async function handleAppUpdate(data) {
        ...
        try {
            ...
            setIsLoading(true);
            const res = await request(`https://api.quickvalide.com/api/Setting/${appId}`, 'PUT', {
                name: data.name,
                domain: data.name
                    ?.trim()
                    ?.toLowerCase()
            ...
            if (!res.status) throw new Error('An error occured while updating the app');
    ...
    async function updateAppStatus() {
        try {
            ...
            if (!res.status) throw new Error('An error occured while changing the app status');
    ...
    return (
        <div className='w-full flex flex-col gap-8 mt-3 pb-6  px-6 md:px-12 lg:px-24 text-text-primary'>
            ...
            <form className='flex flex-col gap-2' onSubmit={handleSubmit(handleAppUpdate)}>
